...
def read_ecb_plus_doc(selected_sent_list, doc_filename, doc_id, file_obj,extracted_mentions,
    ...
    for mid in mid_to_tid_dict:
        ...
        if parse_all or sent_id in selected_sent_list:
            ...
            uni_tokens_str = [t.decode("utf-8") for t in tokens_str]
            mention_obj = MentionData(doc_id, sent_id, token_numbers, ' '.join(uni_tokens_str),
    ...
    for token in root.findall('token'):
        ...
        if token.rel_id is not None:
            file_obj.write(doc_id  + '\t' + str(sent_id) + '\t' + str(token_id) + '\t' + text.decode("utf-8")  + '\t' + \
                            token.rel_id[0] + '\n')
        else:
            file_obj.write(doc_id  + '\t' + str(sent_id) + '\t' + str(token_id) + '\t' + text.decode("utf-8")  + '\t-' + '\n')
